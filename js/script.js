...
// Se llama a la funcion "organizar" si se hace click en el boton correspondiente
document.getElementById('boton-organizador').addEventListener('click', () => organizar("lista"));

// Se llama a la funcion exportar a excel si el usuario da click en el boton correspondiente
...
// Funcion que se encarga de generar el numero con el inidice +1 de (USA), si el usuario dio click en el boton correspondiente
function numUsa (){        
...
// Funcion que se encarga de generar el numero con el indice +57 (Colombia), si el usuario dio click en el boton correspondiente
function numCol (){ 
...
function numAleatorio (indicePrefijo){ // Generamos el resto del numero de manera aleatoria
    let min = 1000000;
    let max = 9999999;
    if (indicePrefijo.includes("+1 ")) { // Validamos que si es un numero de USA solo se generen ciertos limites ya que en USA así es el formato
        min = 2000000;
        max = 7000000; 
    }  // Rango de combinaciones validas tanto para Colombia como para USA.(evitar los numeros 555 que se generan despues del CDA para USA)
    console.log(min);
    console.log(max);
    for (let i = 0; i < 500; i ++){ 
    // Limite por defecto en 500* lo puden cambiar si desean generar mas numeros de una sola tanda                           
...
function comprobacion (numeroCompleto){ // Se encarga de comprobar que no se generen numeros repetidos
    if (generados.has(numeroCompleto)) { // En caso de que ya hallan sido generados no avanzamos y seguimos con el otro
        return;
    } else {
        generados.add(numeroCompleto); //si no han sido generados los agreamos al set, para futuras validaciones de ese numero
